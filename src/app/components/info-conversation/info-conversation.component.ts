...
export class InfoConversationComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.uploadService.getFiles(this.roomchatID, 'image',this.previousTimeSeq,10).subscribe(data => {
      if (data && data.success) {
  ...
  showPhotoInViewMode(imageID){
    this.getPhotoViewModeSubscription = this.uploadService.getFile(this.roomchatID,'image',imageID).subscribe(data => {
      if(data && data.success){
