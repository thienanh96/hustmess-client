import { Component, OnInit, ChangeDetectorRef, HostListener, OnDestroy, AfterViewInit } from '@angular/core';
...
export class ConversationComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  onResize(event) {
    ...
    let conversationHeaderHeight = document.getElementById('conversation-header').clientHeight;
    ...
    let conversationWidth = document.getElementById('conversation-header').clientWidth - 110;
    if(this.showNavPrevious){
      conversationWidth -= 40;
    }
    document.getElementById('conversation-header-username').setAttribute('style', 'width: ' + conversationWidth + 'px');
  ...
  ngOnInit() {
    ...
    this.a = this.roomchatService.getRoomchat(this.roomchatID).subscribe(data => {
      if (data && data.success) {
        ...
        this.b = this.roomchatUserService.getRoomchatUsers(this.roomchatID).subscribe(dataa => {
          if (dataa && dataa.success) {
            this.usersInRoomchat = dataa.roomchatUsers.map(el => el.userID);
        ...
        this.c = this.messageService.getMessages(this.roomchatID, this.previousTimeSeq, 20).subscribe(data => {
          if (data.success) {
            let index = 0;
            let previousUserID = '';
    ...
    this.d = this.componentCommunicationService.getData().subscribe(data => {
      ...
      if (data.fromComponent === 'roomchat') {
        ...
        if (data.status === 'sending') {
          ...
          let conversationHeight = document.getElementById('conversation-body').clientHeight - 100;
          document.getElementById('conversation-body').setAttribute('style', 'height: ' + conversationHeight + 'px');
        }

      } else if (data.fromComponent === 'app' && data.type === 'idle') {
        if (this.usersInRoomchat.length !== 0 && this.usersInRoomchat.includes(data.fromUserID + '')) {
          this.isActive = false;
          this.isIdle = true;
        }
      ...
      } else if (data.fromComponent === 'app' && data.type === 'receive-message') {
        if (data.data.roomchatID !== this.roomchatID) return;
        if (data.data.type === 'confirm') {
          this.confirmation.sent = true;
          this.confirmation.sending = false;
        } else if (data.data.type === 'message') { //khi co tin nhan moi den
          this.roomchatService.resetSeenUsers(this.roomchatID).subscribe(data => {})
      ...
      } else if (data.fromComponent === 'last-message' && data.type === 'delete-all-messages') {
        if (data.roomchatID !== this.roomchatID) return;
        this.messages = [];
        this.e = this.messageService.getMessages(this.roomchatID, this.previousTimeSeq, 15).subscribe(data => {
          if (data.success) {
            for (let message of data.messages) {
              this.messages.push({
                userID: message.userProfile._id,
      ...
      } else if (data.fromComponent === 'app' && data.type === 'confirm-received-message') {
        if (data.data.roomchatID !== this.roomchatID) return;
        this.confirmation.received = true;
        this.confirmation.sent = false;
        this.confirmation.sending = false;
        this.confirmation.seen = false;
      } else if (data.fromComponent === 'app' && data.type === 'confirm-seen-message') {
        if (data.data.roomchatID !== this.roomchatID) return;
        if (!this.IDSeenBy.includes(data.data.userID + '')) {
          this.IDSeenBy.push(data.data.userID);
          this.roomchatService.addSeenUsers(this.roomchatID,data.data.userID).subscribe(data => {console.log('add_____',data)})
        }
        this.confirmation.received = false;
        this.confirmation.sent = false;
        this.confirmation.sending = false;
        this.confirmation.seen = true;
      } else if (data.fromComponent === 'app' && data.type === 'check-valid-seen') {
        if (data.data.roomchatID !== this.roomchatID) return;
        let isSentByOther;
        if (this.messages.length !== 0) {
          isSentByOther = this.messages[this.messages.length - 1].userID + '' !== this.myID + '';
        } else {
          isSentByOther = false;
        }
        if (isSentByOther) {
  ...
  seeWhoSeen() {
    ...
    this.getUserSubscription = this.userService.getUsersFromIDs(this.IDSeenBy).subscribe(data => {
  ...
  showOrHideInfo() {
    ...
    if (w < 900) {
      infoConvWidth = document.getElementById('conversation-header').clientWidth;
      document.getElementById('info-conversation-component').style.width = infoConvWidth + 'px';
    } else {
      infoConvWidth = 351;
      document.getElementById('info-conversation-component').style.width = infoConvWidth + 'px';
    }
    if (this.showConversationInfo) {
      let conversationSideWidth = document.getElementById('conversation-header').clientWidth - infoConvWidth;
      if (w < 900) {
        hideInputComp = true;
      }
      document.getElementById('conversation-side').style.width = conversationSideWidth + 'px';
  ...
  adjustHTMLResponsive(w) {
    if (w < 900) {
      ...
      let conversationSideWidth = document.getElementById('conversation-header').clientWidth;
      document.getElementById('conversation-side').style.width = conversationSideWidth + 'px';
      document.getElementById('loading-messages').style.width = conversationSideWidth + 'px';
      let conversationUsernameWidth = conversationSideWidth - 150;
      document.getElementById('conversation-header-username').style.width = conversationUsernameWidth + 'px';
    } else {
      this.showConversationInfo = true;
      ...
      document.getElementById('conversation-side').style.width = conversationSideWidth + 'px';
      document.getElementById('loading-messages').style.width = conversationSideWidth + 'px';
      let conversationUsernameWidth = document.getElementById('conversation-header').clientWidth - 110;
      document.getElementById('conversation-header-username').style.width = conversationUsernameWidth + 'px';
  ...
  onScrollMessages() {
    ...
    let scrollHeight = document.getElementById('conversation-body').scrollHeight;
    let clientHeight = document.getElementById('conversation-body').clientHeight;
    if (
      scrollTop < 10 &&
      this.continueToLoad
    ) {
      ...
      let conversationSideWidth = document.getElementById('conversation-header').clientWidth - 351;
      document.getElementById('loading-messages').style.width = conversationSideWidth + 'px';
      this.c = this.messageService.getMessages(this.roomchatID, this.previousTimeSeq, 4).subscribe(data => {
        if (data.success) {
          this.loadingMessages = false;
